...
in_topics_path = './data/inequality/abstract_topTFIDF_title.csv' # we are using this to filter out any wiki titles that exactly match labels from the topics
...
print("after removing duplicates, there are ", len(data), 'data left')
...
stop_words_for_title = stopwords.words('english')
print('before extend, stopwords for title are:', len(stop_words_for_title))
stop_words_for_title.extend(['from', 'subject', 'study', 'research', 'review', 'impact', 'evaluation', 'trial', 'one',
                             'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'comparison',
                             'use', 'using', 'during', 'for', 'in', 'to', 'making', 'effect', 'what', 'effectiveness',
                             'methods'])
print('before extend, stopwords for title are:', len(stop_words_for_title))
stop_words_for_title = set(stopwords.words('english'))
...
def text_cleaner(text, num):
    ...
    newString = re.sub('[m]{2,}', 'mm', newString)
    if (num == 0):
        tokens = [w for w in newString.split() if not w in stop_words_for_title]
    else:
        tokens = newString.split()
    long_words = []
    for i in tokens:
...
for i in range(len(cleaned_titles)):
    if len(cleaned_titles[i].split()) <= SHORT_TITLE:
